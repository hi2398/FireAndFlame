...
std::shared_ptr<EState> RoamingState::Update(Enemy& enemy)
{
	...
	if (enemy.GetWallCollisionLeft() && enemy.GetDirection() == LEFT && enemy.GetEnemyType() != EnemyTypes::SpiderBot) {
		enemy.SetDirection(RIGHT);
	}
	if (enemy.GetWallCollisionRight() && enemy.GetDirection() == RIGHT && enemy.GetEnemyType() != EnemyTypes::SpiderBot) {
		enemy.SetDirection(LEFT);
	}

	if (enemy.GetEnemyType() != EnemyTypes::Flyer && enemy.GetEnemyType() != EnemyTypes::SpiderBot) {
