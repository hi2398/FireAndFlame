...
std::shared_ptr<EState> IBMelee::Update(Enemy &enemy) {
    ...
    --meleeTimer;
    if (meleeTimer<=20){
        texRec={static_cast<float>(96*iceBoss->GetPhase()), static_cast<float>(96*iceBoss->GetPhase())+128, 64, 32};
        ...
        texRec={static_cast<float>(96*iceBoss->GetPhase()), static_cast<float>(96*iceBoss->GetPhase())+96, 64, 32};
    } else {
        texRec={static_cast<float>(96*iceBoss->GetPhase()), static_cast<float>(96*iceBoss->GetPhase())+64, 64, 32};
    }
