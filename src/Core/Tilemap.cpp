    ...
    for (auto const &layer : levelDescription["layers"]) {
        if (layer["type"] == "tilelayer" && layer["visible"]) {
            for (auto const &tileId : layer["data"]) {
                if (tileId != 0) {
                    tilemapPos.x =  (float) ((int) tileId - 1 % (int) tilesetDescription["columns"]) *
                                 (float) levelDescription["tilewidth"];
                    tilemapPos.y =  (float) floor((float) tileId / (float) tilesetDescription["columns"]) *
                                 (float) levelDescription["tilewidth"];
